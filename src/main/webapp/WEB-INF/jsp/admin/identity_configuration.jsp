...
$(function(){
	...
	$(".identity_configuration_save").click(function(){
		...
		$.ajax({
		   type: "POST",
		   url: "${ctx}/admin/config/saveUserType.json",
		   dataType: "json",
...
function identity_configuration_li_del(id){
	...
	$(".confirm_li.confirm").off('click').on("click",function(){
		$.ajax({
			   type: "POST",
			   url: "${ctx}/admin/config/deleteUserType.json",
			   dataType: "json",
			   data: {id:id},
			   success: function(msg){
				   console.log(msg);
...
<div class="back_right_box">
	<h1 class="identity_configuration_header">身份配置</h1>
	
	<div class="identity_configuration_content">
		<span class="identity_configuration_name">身份名称</span><input type="text" placeholder="请输入身份名称" class="identity_configuration_value"><span class="identity_configuration_save">保存</span>
	</div>
		...
		<ul class="identity_configuration_ul">
			<c:forEach items="${userTypes}" var="userType">
				<li class="identity_configuration_li">
					...
					<span class="identity_configuration_li_value">${userType.name}</span>
					<span class="identity_configuration_li_del" onclick="identity_configuration_li_del('${userType.id}')">删除</span>
				</li>
			</c:forEach>
