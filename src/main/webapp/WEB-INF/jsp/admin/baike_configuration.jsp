...
$(function(){
	...
	$(".back_nav_li").eq(6).find(".back_article_li").eq(4).addClass("index"); */
	...
	$(".identity_configuration_save").click(function(){
		...
		$.ajax({
		   type: "POST",
		   url: "${ctx}/admin/config/saveArticleCategory.json",
		   dataType: "json",
...
function identity_configuration_li_del(id){
	$(".confirm_box").show();
	$(".bg").show();
	$(".confirm_header").text("您确认删除该干货类别吗？");
	
	$(".confirm_li.confirm").off('click').on("click",function(){
		$.ajax({
			   type: "POST",
			   url: "${ctx}/admin/config/deleteArticleCategory.json",
			   dataType: "json",
			   data: {id:id},
			   success: function(msg){
				   console.log(msg);
...
<div class="back_right_box">
	<h1 class="identity_configuration_header">干货类别配置</h1>
	
	<div class="identity_configuration_content">
		<span class="identity_configuration_name">干货类别名称</span><input type="text" placeholder="请输入干货类别名称" class="identity_configuration_value"><span class="identity_configuration_save">保存</span>
	</div>
		...
		<div class="identity_configuration_content list">
			...
			<c:forEach items="${articleCategories}" var="articleCategory">
				<li class="identity_configuration_li">
					<span class="identity_configuration_li_name">干货类别名称</span>
					<span class="identity_configuration_li_value">${articleCategory.name}</span>
					<span class="identity_configuration_li_del" onclick="identity_configuration_li_del('${articleCategory.id}')">删除</span>
				</li>
			</c:forEach>	
			</ul>
		</div>
