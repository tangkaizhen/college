...
$(function(){
	...
	$(".back_nav_li").eq(6).find(".back_article_li").eq(3).addClass("index"); */
	...
	$(".identity_configuration_save").click(function(){
		...
		$.ajax({
		   type: "POST",
		   url: "${ctx}/admin/config/saveCourseCategory.json",
		   dataType: "json",
...
function identity_configuration_li_del(id){
	$(".confirm_box").show();
	$(".bg").show();
	$(".confirm_header").text("您确认删除该课程类型吗？");
	
	$(".confirm_li.confirm").off('click').on("click",function(){
		$.ajax({
			   type: "POST",
			   url: "${ctx}/admin/config/deleteCourseCategory.json",
			   dataType: "json",
			   data: {id:id},
			   success: function(msg){
				   console.log(msg);
...
<div class="back_right_box">
	<h1 class="identity_configuration_header">课程类别配置</h1>
	...
	<div class="identity_configuration_content">
		...
		<div>
			<span class="identity_configuration_name">课程类别名称</span><input type="text" placeholder="请输入课程类别名称" class="identity_configuration_value"><span class="identity_configuration_save">保存</span>
		</div>	
	</div>
	<c:if test="${ not empty courseCategories}">
	<div class="identity_configuration_content list">
		...
		<c:forEach items="${courseCategories}" var="courseCategory">
			<li class="identity_configuration_li">
				<span class="identity_configuration_li_iden">${courseCategory.userType}</span>
				<span class="identity_configuration_li_arg">${courseCategory.uavType}</span>
				<span class="identity_configuration_li_value">${courseCategory.name}</span>
				<span class="identity_configuration_li_del" onclick="identity_configuration_li_del('${courseCategory.id}')">删除</span>
			</li> 
			</c:forEach>
		</ul>
	</div>
